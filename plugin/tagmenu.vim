...
"
" Changes : Ajay Kidave(ajayvk@gmail.com) for TAL/pTAL support
"
" Overview
" --------
...
if !exists("Tmenu_ctags_cmd")
    let Tmenu_ctags_cmd = 'c:/ttags/ctags.exe'
endif
...
let s:tmenu_file_types = 'asm asp awk c cpp cobol eiffel fortran java lisp ' .
            \ 'make pascal perl php python rexx ruby scheme sh slang tal tcl ' .
            \ 'verilog vim yacc'
...
"
" tal language
let s:tmenu_tal_ctags_args = ''
let s:tmenu_tal_tag_types = 'proc define literal struct subproc section'
...
function! s:Add_Tags_Menu(menu_clear)
    ...
    if ftype == 'tal' 
        if g:Tmenu_sort_type == "name"
          let ctags_cmd = "c:/ttags/taltags.exe -o -d" . ' "' . filename . '"'
        else
          let ctags_cmd = "c:/ttags/taltags.exe -o -n -d" . ' "' . filename . '"'
        endif
